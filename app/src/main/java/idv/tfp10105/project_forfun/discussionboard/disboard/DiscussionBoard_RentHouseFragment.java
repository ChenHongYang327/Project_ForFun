...
//            Toast.makeText(activity, "沒有貼文", Toast.LENGTH_SHORT).show();
        ...
        if (rentAdapter == null) {
            rv_rent.setAdapter(new RentAdapter(activity, posthomeList));
        } else {
            //更新Adapter資料,重刷
            rentAdapter.setAdapter(posthomeList);
    ...
    private class RentAdapter extends RecyclerView.Adapter<RentAdapter.MyViewHolder> {
        ...
        public RentAdapter(Context context, List<Posthome> posthomeList) {
